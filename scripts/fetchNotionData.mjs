...
const WEIGHT_DB_ID =
  process.env.NOTION_WEIGHT_DB_ID ?? '335fb4c819b38066beb0ffcb222759bd';
...
async function fetchWeightRecords() {
  const pages = await fetchAllPages(WEIGHT_DB_ID);
  console.log(`Found ${pages.length} pages in Notion weight DB.`);
  ...
  for (const page of pages) {
    ...
    // Extract kg property (number field)
    const kgProp =
      page.properties?.kg ??
      page.properties?.Kg ??
      page.properties?.['체중(kg)'] ??
      page.properties?.['몸무게'];
    let kg = null;
    if (kgProp?.type === 'number' && kgProp.number !== null) {
      kg = kgProp.number;
    }

    if (!date || kg === null) continue;

    records.push({ id: page.id, date, kg });
  }
...
async function main() {
  ...
  console.log('Fetching weight records from Notion…');
  const weightRecords = await fetchWeightRecords();
  console.log(`Generated ${weightRecords.length} weight record entries.`);
...
export const weightRecords: WeightRecord[] = ${JSON.stringify(weightRecords, null, 2)};
`;
